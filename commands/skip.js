...
module.exports.run = async (bot, message, args, queue) => {
  ...
  if(!serverQueue.songs[1]) return message.channel.send(new Discord.MessageEmbed().setTitle("There Are No Songs To Skip!").setColor(0xff0000).setFooter(message.guild.me.displayName).setTimestamp());
  serverQueue.connection.dispatcher.end();
  message.channel.send(new Discord.MessageEmbed().setTitle(`⏩ Skipped \`${serverQueue.songs[0].title}\` ⏩`).setColor(0xff6200).setFooter(message.guild.me.displayName).setTimestamp());
};
