...
module.exports.run = async (bot, message, args, queue) => {
  ...
  if(!serverQueue) return message.channel.send(new Discord.MessageEmbed().setTitle("There Are No Songs In The Queue!").setColor(0xff0000).setFooter(message.guild.me.displayName).setTimestamp());
  if(!serverQueue.songs[1]) return message.channel.send(new Discord.MessageEmbed().setTitle("There's Nothing Playing!").setColor(0xff0000).setFooter(message.guild.me.displayName).setTimestamp());
