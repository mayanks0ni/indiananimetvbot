...
module.exports.run = async (bot, message, args) => {
 ...
 if(!user) return message.channel.send(new Discord.MessageEmbed().setTitle("This Command Is Used Like This `+warnlog [user]`!").setFooter(message.guild.me.displayName).setTimestamp().setColor(0xff0000));
 warnLog.all(`SELECT * FROM warnlog WHERE userid = '${user.id}'`, (err, rows) =>{
    if(err){
        console.error(err);
        return;
    }
    if(rows.length < 1) return message.channel.send(new Discord.MessageEmbed().setTitle("No Warnings Found For That User!").setColor(0xff0000).setFooter(message.guild.me.displayName).setTimestamp());
    var i;
