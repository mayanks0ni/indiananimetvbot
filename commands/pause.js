...
module.exports.run = async (bot, message, args, queue) => {
  ...
  if(!serverQueue.songs[0]) return message.channel.send(new Discord.MessageEmbed().setTitle("There Are No Songs To Pause!").setColor(0xff0000).setFooter("IAT Bot").setTimestamp());
  serverQueue.connection.dispatcher.pause();
  message.channel.send(new Discord.MessageEmbed().setTitle(`⏸️ Paused \`${serverQueue.songs[0].title}\` ⏸️`).setColor(0x86ff00).setFooter("IAT Bot").setTimestamp());
};
