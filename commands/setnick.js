...
module.exports.run = async (bot, message, args) => {
	...
	if (!message.member.permissions.has("MANAGE_NICKNAMES")) {
		...
		if (!member12 || !nickname) {
			...
			return message.channel.send(new Discord.MessageEmbed().setTitle(`Successfully Changed ${member12.displayName} To ${nickname}!`).setColor("RANDOM").setFooter(message.guild.me.displayName).setTimestamp());
		} catch(e){
			console.log(e);
			return message.channel.send(new Discord.MessageEmbed().setTitle("Error! Cannot Change The Nickname Of That User! \n I Can't Change The Nickname Of Someone With Higher Role Than Me!").setColor(0xff0000).setTimestamp().setFooter(message.guild.me.displayName))
		}
		}
	}
};
