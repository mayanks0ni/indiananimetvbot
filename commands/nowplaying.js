...
module.exports.run = async (bot, message, args, queue) => {
  ...
  if(!serverQueue) return message.channel.send(new Discord.MessageEmbed().setTitle("There's Nothing Playing Right Now!").setColor(0xff0000).setFooter("IAT Bot").setTimestamp());
  let title;
  ...
  message.channel.send(new Discord.MessageEmbed().setTitle(`Currently Playing \n\`${serverQueue.songs[0].title}\`!`).addField("**Upcoming**", `\`${title}\``).setColor(0xc5ff00).setFooter("IAT Bot").setTimestamp());
};
