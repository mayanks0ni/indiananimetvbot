...
module.exports.run = async (bot, message, args, queue) => {
  ...
  if(!serverQueue.songs[0]) return message.channel.send(new Discord.MessageEmbed().setTitle("There Are No Songs To Resume!").setColor(0xff0000).setFooter("IAT Bot").setTimestamp());
  serverQueue.connection.dispatcher.resume();
  message.channel.send(new Discord.MessageEmbed().setTitle(`▶️ Resumed \`${serverQueue.songs[0].title}\` ▶️`).setColor(0x00ffc1).setFooter("IAT Bot").setTimestamp());
};
