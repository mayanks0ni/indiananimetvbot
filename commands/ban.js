...
module.exports.run = async (bot, message, args) => {
	message.delete()
	if (!message.member.permissions.has("BAN_MEMBERS")) return message.channel.send(new Discord.MessageEmbed().setTitle('You Don\'t Have Enough Permissions!').setColor(0xff0000).setFooter("IAT Bot").setTimestamp());
	const user = message.mentions.users.first();
	const reason = args.slice(2).join(" ");
	if (!reason || !user) return message.channel.send(new Discord.MessageEmbed().setTitle('This Command Is Used Like This \`+ban [user] [reason]\`').setColor(0xff0000).setFooter("IAT Bot").setTimestamp());
	if (user) {
